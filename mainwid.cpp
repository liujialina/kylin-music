...
MainWid::MainWid(QStringList str, QWidget *parent)
    : QMainWindow(parent)
{
...
void MainWid::Single(QStringList path)   //单例
{
    QString str;
    if(path.size() > 1)
    {
...
void MainWid::client_get(QString str)
{
...
void MainWid::initAction()//初始化事件
{
    ...
    connect(myPlaySongArea->lastBtn,SIGNAL(clicked()),this,SLOT(playPre())); //上一首
//    connect(myPlaySongArea->lastBtn,SIGNAL(clicked()),this,SLOT(on_playlist_lastBtn_clicked())); //上一首
    connect(myPlaySongArea->nextBtn,SIGNAL(clicked()),this,SLOT(playNext())); //下一首
...
//    connect(mySideBar->myMusicListWid->Music,&QMediaPlayer::stateChanged,this,&MainWid::Music_stateChang);
    ...
    //迷你模式槽函数绑定
    connect(m_MiniWidget->m_nextBtn,&QPushButton::clicked,this,&MainWid::playNext);
    connect(m_MiniWidget->m_playStateBtn,&QPushButton::clicked,this,&MainWid::play_Song);
    connect(m_MiniWidget->m_preBtn,&QPushButton::clicked,this,&MainWid::playPre);
...
void MainWid::deleteAllItem()
{
    QList<SongItem*> list1 = this->findChildren<SongItem*>();
    for(SongItem *tmp :list1)
    {
        tmp->deleteLater();
    }
    QList<HistoryListItem*> list2 = this->findChildren<HistoryListItem*>();
    for(HistoryListItem *tmp :list2)
    {
        tmp->deleteLater();
    }
}
...
void MainWid::initSystemTray()//初始化托盘
{
    ...
    connect(m_MiniWidget->playMode_Random_Action,&QAction::triggered,[&]()
    {
        ...
        m_MiniWidget->m_orderBtn->setIcon(QIcon::fromTheme("media-playlist-repeat-one"));
        m_MiniWidget->m_orderBtn->setStyleSheet("QPushButton{background:transparent;}"
                                                "QPushButton::hover{border-image:url(:/img/hover/oneloop.png);}"
                                                "QPushButton::pressed{border-image:url(:/img/clicked/oneloop.png);}");
...
int MainWid::kylin_music_play_request(QString cmd1, QString cmd2, QString cmd3)
{
    //990
    ...
    if(cmd1=="-n"||cmd1=="-next")//下一首
    {
        //------下一首-------
        playNext();
        return 0;
    }
    if(cmd1=="-b"||cmd1=="-back")//上一首
    {
        //-------上一首------
        playPre();
        return 0;
    ...
    else
    {
        ...
        if(file.exists())
        {
            ...
            if(ret == DB_OP_SUCC)
            {
                myPlaySongArea->songText(myPlaySongArea->fileData.title); // 正在播放
                m_MiniWidget->songText(myPlaySongArea->fileData.title);   //mini正在播放
//                updataplaylistwidget(currentIndex);
                for(int i = 0;i < mySideBar->playListName.size();i++)
                {
                    if(mySideBar->musicListChangeWid[0]->localAllMusicid.indexOf(myPlaySongArea->fileData.filepath) != -1)
                    {
...
//    connect(mySideBar->myMusicListWid->PlayList, &QMediaPlaylist::currentIndexChanged, this,&MainWid::currentPlayHighlight);
    ...
    qDebug()<<"listcurrent : "<<mySideBar->currentMusicPlaylist;
...
//            mySideBar->myMusicListWid->PlayList->setCurrentIndex(preIndex);
            mySideBar->myMusicListWid->PlayList->previous();
            mySideBar->myMusicListWid->Music->play();
            isPlay = true;
...
//            mySideBar->musicListChangeWid[mySideBar->currentMusicPlaylist]->PlayList->setCurrentIndex(preIndex);
            mySideBar->musicListChangeWid[mySideBar->currentMusicPlaylist]->PlayList->previous();
            mySideBar->musicListChangeWid[mySideBar->currentMusicPlaylist]->Music->play();
            isPlay = true;
...
//            myPlaySongArea->mybeforeList->PlayList->setCurrentIndex(preIndex);
            myPlaySongArea->mybeforeList->PlayList->previous();
            myPlaySongArea->mybeforeList->Music->play();
            isPlay = true;
...
//        nextIndex = (mySideBar->myMusicListWid->PlayList->currentIndex() + 1)%mySideBar->myMusicListWid->PlayList->mediaCount();
        mySideBar->myMusicListWid->PlayList->next();
//        nextIndex = mySideBar->myMusicListWid->PlayList->nextIndex();
//        qDebug()<<"-----"<<mySideBar->myMusicListWid->PlayList->nextIndex(0);
        mySideBar->myMusicListWid->Music->play();
        isPlay = true;
...
//            nextIndex = (mySideBar->musicListChangeWid[mySideBar->currentMusicPlaylist]->PlayList->currentIndex() + 1) % mySideBar->musicListChangeWid[mySideBar->currentMusicPlaylist]->PlayList->mediaCount();
            mySideBar->musicListChangeWid[mySideBar->currentMusicPlaylist]->PlayList->next();
...
//            nextIndex = (myPlaySongArea->mybeforeList->PlayList->currentIndex() + 1) % myPlaySongArea->mybeforeList->PlayList->mediaCount();
            myPlaySongArea->mybeforeList->PlayList->next();
...
void MainWid::moveMainWid(QString newWidth, QString newHeight)
{
...
void MainWid::playMode_Sequential()
{
//    if(mySideBar->currentMusicPlaylist == -2)
//    {
//        return;
...
void MainWid::playMode_CurrentItemInLoop()
{
//    if(mySideBar->currentMusicPlaylist == -2)
//    {
//        return;
...
void MainWid::increaseVolume()   //音量调高
{
    if(mySideBar->currentMusicPlaylist == -2)
    {
        return;
...
void MainWid::reduceVolume()     //音量调低
{
    if(mySideBar->currentMusicPlaylist == -2)
    {
        return;
...
//    {
//        if(args[i].split(".").last() != "mp3" || args[i].split(".").last() != "ogg" ||
//                args[i].split(".").last() != "wma" || args[i].split(".").last() != "spx" ||
//                args[i].split(".").last() != "flac")
//        {
//            qDebug()<<"----last"<<args[i].split(".").last();
//            qDebug()<<"----first"<<args[i].split(".").first();
    ...
    if(mySideBar->currentMusicPlaylist == -1)
    {
        ...
        if(!args.isEmpty())
        {
            ...
            for(int i = 0; i < args.size(); i++)
            {
                ...
                if(mySideBar->myMusicListWid->musicdataStruct.filetype == "mp3" || mySideBar->myMusicListWid->musicdataStruct.filetype == "ogg" ||
                        mySideBar->myMusicListWid->musicdataStruct.filetype == "wma" || mySideBar->myMusicListWid->musicdataStruct.filetype == "spx"||
                        mySideBar->myMusicListWid->musicdataStruct.filetype == "flac")
                {
